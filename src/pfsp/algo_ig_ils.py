...
def _neh_build_from_order(p_times: np.ndarray, order: np.ndarray) -> np.ndarray:
    seq = np.empty(0, dtype=np.int64)
    for job in order:
        best_pos, _ = _best_insert_pos_with_ties(p_times, seq, int(job))
...
def _neh_multi_start(p_times: np.ndarray) -> np.ndarray:
    best_seq: Optional[np.ndarray] = None
    best_val: Optional[int] = None
    for order in _candidate_orders(p_times):
        seq = _neh_build_from_order(p_times, order)
...
class IteratedGreedyILS:
    def __init__(
        ...
        p_times: np.ndarray,
        mechanism: str = "fixed",
        # mechanism/scheduler hyperparams:
        window_size: int = 50,
        p_min: float = 0.10,
    ...
    def _local_search_fixed(self, perm: np.ndarray, value: int, deadline: Optional[float]) -> Tuple[np.ndarray, int]:
        ...
        while True:
            ...
            if neigh is not None and neigh[1] < current_val:
                current_perm, current_val = neigh; improved = True
            if deadline and time.time() >= deadline:
                break
    ...
    def _local_search_adaptive(self, perm: np.ndarray, value: int, deadline: Optional[float]) -> Tuple[np.ndarray, int]:
        ...
        for _ in range(self.ls_step_cap):
            if deadline and time.time() >= deadline:
                break
    ...
    def run(
        ...
        perm = _neh_multi_start(self.p_times)
