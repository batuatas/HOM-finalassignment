...
def main():
    ...
    p.add_argument("--optimistic-init", type=float, default=0.01)
    # neighborhoods
    p.add_argument("--block-lengths", type=str, default="2,3")
    p.add_argument("--granular-window", type=int, default=10)
    p.add_argument("--critical-only", action="store_true")
    p.add_argument("--critical-take-frac", type=float, default=0.25)
    ...
    algo_kwargs = dict(
        ...
        granular_window=args.granular_window,
        critical_only=bool(args.critical_only),
        critical_take_frac=args.critical_take_frac,
        d_frac=args.d_frac,
    ...
    (outdir / "raw.csv").write_text(df.to_csv(index=False))

    if bk:
        df["best_known"] = df["instance"].map(bk)
        df = add_rpd_column(df, best_known=bk)
        (outdir / "raw_with_rpd.csv").write_text(df.to_csv(index=False))

    summ = summarise_by_instance(df)
    (outdir / "summary_by_instance.csv").write_text(summ.to_csv(index=False))
    ...
    (outdir / "overall.csv").write_text(overall.to_csv())
